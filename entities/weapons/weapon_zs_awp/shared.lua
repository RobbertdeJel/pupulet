...
SWEP.IronSightsPos 			= Vector (-7.6111, -12, 2.092)
FrameTime() * 30
...
function ScoutCalcView( pl, pos, angles, fov )
		...
		if IsValid( wep ) and wep:GetClass() == "weapon_zs_awp" and pl:KeyDown( IN_ATTACK2 ) then 
			local SCOPE = {}
			SCOPE.origin = pos
			SCOPE.angles = angles
			SCOPE.fov = fov - 50
...
hook.Add( "CalcView", "awpvieuw", ScoutCalcView )
